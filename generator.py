...
가공 금지: 보도자료에 없는 내용(ESG 경영, 문화 민주화, 독자 반응, 관계자 인터뷰 등)을 추가하지 마십시오.
길이 절대 규칙: 교정 후 본문은 반드시 {min_len}자 이상이어야 합니다. 부족하면 보도자료에서 누락된 정보를 추가하십시오."""
...
**기사 본문 길이 절대 규칙**: 본문(제목·부제 제외)은 반드시 **{min_length}자 이상** 작성하십시오. 이 기준에 미달하면 보도자료에서 아직 반영하지 않은 정보(수상 내역, 도서 가격, 화가 이름, 목차 구성 등)를 추가하여 분량을 확보하십시오.
