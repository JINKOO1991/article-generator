...
class ArticleGenerator:
    ...
    def _api_call_with_retry(self, max_retries: int = 3, **kwargs):
        """API 호출을 rate limit 재시도 로직과 함께 수행한다. response 객체를 반환."""
...
## 번역투 회피
LLM 생성 시 흔히 나타나는 번역투를 피하십시오. DATA 기사에는 번역투가 없습니다.
- 이중피동(보여지다/쓰여지다) → 능동문
- "-에 의해" → 주어 조사
- 물주구문(무생물 주어 + 타동사) → 부사어화
- 불필요한 '이/그/저' 지시어 삭제
- 접속 부사를 기계적으로 삽입하지 말 것
- '-ㄹ 수 있다' 과용 회피
...
예시:
- (나쁜 예) "A기업이 환경, 사회, 거버넌스 분야에서 다양한 활동을 했다"
  → 이것은 보도자료 요약이지 테마가 아닙니다.
- (좋은 예) "A기업은 지역사회 밀착형 ESG를 통해 사업장이 곧 공동체의 일부임을 증명하고 있다"
  → 모든 세부 사례가 이 한 문장을 뒷받침하는 근거가 됩니다.

### B. 문단별 논리 전개도(Outline)
...
1. **단순 병렬 나열(Cataloging) 금지.**
   ...
   "A사업을 했다. B사업도 했다. C사업도 했다"는 카탈로그입니다.

2. **논리적 흐름 필수.**
   앞 문단의 결과가 뒷 문단의 원인이 되어야 합니다.
   예: "지역사회 투자(문단3)의 성과 → 그 기반 위에서 확장된 글로벌 협력(문단4)"
   문단과 문단 사이에 "왜 이 순서인가?"에 답할 수 있어야 합니다.

3. **모든 세부 정보는 핵심 테마에 복무.**
   각 문단에 배치되는 사업·수치·인용문이 핵심 테마를 어떻게 뒷받침하는지
   전개도에 명시하십시오.

4. **보도자료에 없는 내용을 기획안에 포함하지 마십시오.**
   보도자료에 없는 독자 반응, 인물의 동기·감정, 사회적 의미 부여 등을
   기획안에 넣으면 기사 작성 시 그대로 가공됩니다.
{para_info}
...
서론:
  문단 1: [역할] — [내용 요약] — [연결 논리: 기사의 출발점]
  문단 2: [역할] — [내용 요약] — [연결 논리: 문단1에서 제기된 ...]

본론:
  (중간제목: ...)
  문단 3: [역할] — [내용 요약] — [연결 논리: 문단2의 ...를 구체화]
  문단 4: [역할] — [내용 요약] — [연결 논리: 문단3의 성과가 ...]
  ...

결론:
  문단 N: [역할] — [내용 요약] — [연결 논리: 전체 논의를 ...]
"""
...
### D. 기술적 오류
1. 번역투가 검출되는가? (이중피동, "-에 의해", 물주구문, 불필요한 지시어)
2. LLM 패턴이 검출되는가? (접속 부사 기계적 삽입, '-ㄹ 수 있다' 과용)
3. AI 어시스턴트의 중립적·교시적 말투가 섞였는가?
...
3. **번역투 제거**: 이중피동, "-에 의해", 물주구문, 접속 부사 기계적 삽입 제거
...
4. **데이터 과포화 방지 및 논리적 전환 강제:**
   ...
   - 문단과 문단이 전환될 때, 독립된 주제로 갑자기 넘어가는 것을 금지합니다.
     ...
     **논리적 이음새(담화 표지)**를 반드시 포함하십시오.
     예: "이러한 지역 투자의 기반 위에서, A기업은 한 걸음 더 나아가 ..."
{metrics_instruction}

**형식**: 제목 → 부제(3줄 이상, DATA 기사 부제 형식) → (빈 줄) → 본문

**기술적 주의**: 번역투(이중피동, "-에 의해", 물주구문), 접속 부사 기계적 삽입, '-ㄹ 수 있다' 과용 회피.
"""
